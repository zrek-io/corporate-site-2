...
export default function ClimateChange() {
  ...
  return (
    <Box>
      <Meta pageTitle="気候変動対策" pageDescription="ゼットレックは気候変動に対する対策事業を展開しています。" />
      <Header />
      <Hero title="気候変動対策" />
      <Box as="main" w="80%" mx="auto">
        <Box display="flex" alignItems="center" justifyContent="center" my={20}>
          ...
          <Box display={{ sm: 'none', lg: 'block' }} w="50%">
            <Image src={climateChangeImage.src} w="75%" mx="auto" alt="気候変動対策" />
          </Box>
        </Box>
        <Box w="100%">
          <Image src={HomeServiceImage.src} w={{ sm: '90%', md: '75%', lg: "45%" }} mx="auto" alt="Zrekの提供サービス" />
        </Box>
        <Box p={{ sm: 6, lg: 12 }} textAlign="center">
          <PrimaryButton title="問い合わせ" width={{ sm: '65%', md: '30%', lg: "17%" }} margin={{ sm: '4', lg: "8" }} onClick={onClickContact} />
          <BorderButton title="もっと詳しく" width={{ sm: '65%', md: '30%', lg: "17%" }} onClick={onClickMore} />
        </Box>
      </Box>
      <Footer />
    </Box>
